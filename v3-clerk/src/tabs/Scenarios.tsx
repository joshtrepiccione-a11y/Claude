...
export function Scenarios({ precincts }: { precincts: PrecinctBaseline[] }) {
  ...
  const comparison = useMemo(() => {
    ...
    return rows.map((r) => {
      const { county, rows: rs } = calculateScenarioResult(precincts, r.assumptions);
    ...
    <div className="max-w-[1500px] mx-auto px-4 py-4 space-y-4">
      <div className="flex items-start justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-sm text-slate-600">Pick a preset, tune assumptions, or ask the model to find a winning path for Bender.</p>
        </div>
      ...
      <Card
        ...
        subtitle="Deterministic heuristic — adjusts a small set of high-leverage knobs in a fixed order until Bender wins."
      >
          ...
          <table className="min-w-full text-sm">
            <thead className="text-left text-xs uppercase text-slate-500">
              <tr>
                ...
                <th className="px-2 py-2">Bender Wins?</th>
            ...
            <tbody>
              {comparison.map((r) => (
                <tr key={r.id} className="border-t border-slate-100">
                  ...
                  <td className="px-2 py-2">{fmtMargin(r.county.scenario.marginPct)}</td>
                  <td className="px-2 py-2">{r.county.electsD ? <Pill tone="green">Yes</Pill> : <Pill tone="slate">No</Pill>}</td>
...
function mainLever(a: ScenarioAssumptions): string {
  const items: Array<[string, number]> = [
    ["County D swing", Math.abs(a.demSwing - a.repSwing)],
...
function ExpertControls({
  ...
  const sliders: Array<[keyof ScenarioAssumptions, string, number, number, number, string]> = [
    ...
    ["demSwing", "Democratic (Bender) swing", -10, 10, 0.1, "pp"],
    ["repSwing", "Republican (Giralo) swing", -10, 10, 0.1, "pp"],
