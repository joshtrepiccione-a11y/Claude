  ...
  function copySummary() {
    const lines = [
      ...
      ...filtered.slice(0, 20).map(
        (r) =>
          `${r.rank}. ${r.baseline.precinctName} (${r.baseline.municipality}) — ${fmtMargin(r.baselineMarginPct)} → ${fmtMargin(r.scenarioMarginPct)}, ${fmtSigned(r.netVoteOpportunity)} net D, ${r.action}`,
      ),
    ];
    ...
    <div className="max-w-[1500px] mx-auto px-4 py-4 space-y-4">
      <div className="flex items-start justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Button variant="secondary" onClick={() => downloadFile("atlantic_clerk_targets.csv", targetsToCSV(filtered))}>
            Export Target CSV
          </Button>
        </div>
      </div>
              ...
              <tbody>
                {filtered.map((r) => (
                  <tr key={r.baseline.precinctId} className="border-t border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="px-2 py-1.5">{fmtMargin(r.baselineMarginPct)}</td>
                    <td className="px-2 py-1.5">{fmtMargin(r.scenarioMarginPct)}</td>
