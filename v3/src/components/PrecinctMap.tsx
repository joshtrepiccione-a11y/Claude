...
export function colorFor(row: PrecinctRow, layer: MapLayer): string {
  switch (layer) {
    ...
    case "candidate-b-margin":
      return marginColor(candidateMargin(row, "B"));
...
function riskColor(r: PrecinctRow): string {
  // Risk = we are leading but margin is fragile (toss-up + low confidence).
  const m = r.scenarioSlateMarginPct;
...
function candidateMargin(r: PrecinctRow, which: "A" | "B"): number {
  const total = r.scenario.demA + r.scenario.demB + r.scenario.repA + r.scenario.repB;
  if (total === 0) return 0;
  if (which === "A") return ((r.scenario.demA - r.scenario.repA) / (r.scenario.demA + r.scenario.repA || 1)) * 100;
  return ((r.scenario.demB - r.scenario.repB) / (r.scenario.demB + r.scenario.repB || 1)) * 100;
}
