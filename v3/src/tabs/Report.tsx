...
import { DISTRICT } from "../lib/data/config";
...
export function Report({
  ...
  const memo = useMemo(
    () => generateMemo(preset ?? null, state.assumptions, district, rows),
    [preset, state.assumptions, district, rows],
  );

  const memoText = useMemo(
    () =>
      [
        `${DISTRICT.displayName} — ${DISTRICT.subtitle}`,
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-4 space-y-4">
      ...
      <Card className="print-page" title={DISTRICT.displayName} subtitle={`${DISTRICT.subtitle} · Scenario: ${memo.scenarioName}`} right={<Pill tone="amber">Modeled</Pill>}>
