...
export function Data({ rows, warnings }: { rows: PrecinctRow[]; warnings: string[] }) {
  ...
  const muniCoverage = useMemo(() => {
    ...
    return LD8_MUNICIPALITIES.map((m) => ({
      ...m,
  ...
  function previewCSV(file: File) {
    ...
    reader.onload = () => {
      ...
      const required = [
        ...
        "registered_voters",
        "baseline_turnout",
        "baseline_dem_candidate_a_votes",
        "baseline_dem_candidate_b_votes",
        "baseline_rep_candidate_a_votes",
        "baseline_rep_candidate_b_votes",
        "election_day_votes",
        "early_votes",
        "vbm_votes",
      ];
      ...
      for (let i = 1; i < Math.min(lines.length, 11); i++) {
        ...
        sample.push(cells);
        if (muniIdx >= 0 && !isLD8Municipality(cells[muniIdx])) {
          errors.push(`Row ${i}: municipality "${cells[muniIdx]}" is not in LD8 config`);
        }
      }
  ...
  return (
    <div className="max-w-[1500px] mx-auto px-4 py-4 space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <Card title="Current data source">
          <ul className="text-sm space-y-1 text-slate-700">
            <li><strong>Source:</strong> Modeled precinct GeoJSON (<code>data/ld8_precincts.geojson</code>)</li>
            <li><strong>Precincts:</strong> {rows.length}</li>
            <li><strong>Counties:</strong> Atlantic, Burlington</li>
            <li><strong>Year:</strong> {DISTRICT.year}</li>
          </ul>
        </Card>
        ...
        <Card title="Methodology">
          <ul className="text-sm space-y-1 text-slate-700 list-disc list-inside">
            <li>Baseline = upstream Assembly projection from 2025 results + 2024 presidential cross-pressure.</li>
            <li>Scenario engine is deterministic — see <code>lib/modeling/scenario.ts</code>.</li>
            <li>Candidate placeholder names are configurable in <code>lib/data/config.ts</code>.</li>
      ...
      <Card title="Candidate configuration">
        ...
        <p className="text-xs text-slate-500 mt-2">Update placeholders in <code>src/lib/data/config.ts</code> once candidate filings are certified.</p>
      </Card>

      <Card title="LD8 municipality coverage">
      ...
      <Card
        title="Import data"
        subtitle="Validate a CSV against the V3 schema before swapping it in (preview-only)."
      >
          ...
          {[
            ...
            <div key={label} className="bg-slate-50 rounded p-3 border border-slate-100">
              ...
              <input
                ...
                className="text-xs"
              />
              <div className="text-[11px] text-slate-500 mt-1">Preview-only in V3 prototype.</div>
            </div>
          ))}
        </div>
        {csvPreview && (
      ...
      <Card title="Export current data">
        <div className="flex gap-2 flex-wrap">
          <Button variant="secondary" onClick={() => downloadFile("ld8_precincts.csv", precinctsToCSV(rows))}>
            Export precincts CSV ({fmtNumber(rows.length)} rows)
          </Button>
          <Button variant="ghost" onClick={() => downloadFile("ld8_config.json", JSON.stringify({ DISTRICT, CANDIDATES, LD8_MUNICIPALITIES }, null, 2), "application/json")}>
            Export config JSON
          </Button>
        </div>
      </Card>
