...
export function Targets({ rows }: { rows: PrecinctRow[] }) {
  ...
  const [countyFilter, setCountyFilter] = useState<string[]>(state.countyFilter);
  ...
  const filtered = useMemo(() => {
    ...
    if (countyFilter.length) f = f.filter((r) => countyFilter.includes(r.baseline.county));
    if (categoryFilter.length) f = f.filter((r) => categoryFilter.includes(r.category));
    return f.sort(sorter(sortKey, sortDir));
  }, [allRanked, minNet, muniFilter, countyFilter, categoryFilter, sortKey, sortDir]);
  ...
  function toggleSort(k: SortKey) {
    if (sortKey === k) setSortDir(sortDir === "asc" ? "desc" : "asc");
      ...
      setSortKey(k);
      setSortDir(k === "rank" || k === "precinct" || k === "municipality" || k === "county" || k === "category" ? "asc" : "desc");
    }
  }

  function copySummary() {
    const lines = [
      `Top ${Math.min(20, filtered.length)} target precincts — total net D opportunity ${fmtSigned(totalNet)}`,
      ...filtered.slice(0, 20).map(
        (r) =>
          `${r.rank}. ${r.baseline.precinctName} (${r.baseline.municipality}, ${r.baseline.county}) — ${fmtMargin(r.baselineSlateMarginPct)} → ${fmtMargin(r.scenarioSlateMarginPct)}, ${fmtSigned(r.netVoteOpportunity)} net D, ${r.action}`,
      ),
    ];
  ...
  return (
    <div className="max-w-[1500px] mx-auto px-4 py-4 space-y-4">
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-600">
            Ranked by net Democratic vote opportunity under the active scenario.
          </p>
        </div>
        <div className="flex gap-2 flex-wrap">
          <Button variant="secondary" onClick={copySummary}>Copy Target Summary</Button>
          <Button variant="secondary" onClick={() => downloadFile("ld8_targets.csv", targetsToCSV(filtered))}>
            Export Target CSV
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <Card><Stat label="Total Net D Opportunity" value={fmtSigned(totalNet)} tone="good" /></Card>
      ...
      <Card title="Filters">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          <MultiSelect label="County" options={counties} selected={countyFilter} onChange={setCountyFilter} />
        ...
        {filtered.length === 0 ? (
          ...
          <div className="overflow-auto thin-scroll">
            <table className="min-w-full text-sm">
              <thead className="text-left text-xs uppercase text-slate-500 sticky top-0 bg-white">
                <tr>
                  ...
                  <Th onClick={() => toggleSort("county")} active={sortKey === "county"} dir={sortDir}>County</Th>
                  ...
                  <Th onClick={() => toggleSort("net")} active={sortKey === "net"} dir={sortDir} align="right">Net D</Th>
              ...
              <tbody>
                {filtered.map((r) => (
                  <tr key={r.baseline.precinctId} className="border-t border-slate-100 hover:bg-slate-50">
                    <td className="px-2 py-1.5 text-slate-500">{r.rank}</td>
                    ...
                    <td className="px-2 py-1.5 text-slate-500">{r.baseline.county}</td>
                    <td className="px-2 py-1.5"><Pill tone="navy">{r.category}</Pill></td>
                    <td className="px-2 py-1.5">{fmtMargin(r.baselineSlateMarginPct)}</td>
                    <td className="px-2 py-1.5">{fmtMargin(r.scenarioSlateMarginPct)}</td>
...
function Th({
  ...
  return (
    <th
      onClick={onClick}
      className={`px-2 py-2 cursor-pointer select-none ${align === "right" ? "text-right" : "text-left"} ${active ? "text-slate-900" : ""}`}
    >
...
function sorter(k: SortKey, dir: "asc" | "desc") {
  ...
  return (a: ReturnType<typeof rankTargetPrecincts>[number], b: ReturnType<typeof rankTargetPrecincts>[number]) => {
    switch (k) {
      ...
      case "county": return sgn * a.baseline.county.localeCompare(b.baseline.county);
      case "category": return sgn * a.category.localeCompare(b.category);
      case "baseline": return sgn * (a.baselineSlateMarginPct - b.baselineSlateMarginPct);
      case "scenario": return sgn * (a.scenarioSlateMarginPct - b.scenarioSlateMarginPct);
