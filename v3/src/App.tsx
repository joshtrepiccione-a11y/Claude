...
import { loadLD8, type LoadedData } from "./lib/data/load";
...
function Shell() {
  ...
  useEffect(() => {
    loadLD8().then(setData).catch((e) => setError(String(e)));
  }, []);
  ...
  if (error) {
    return (
      <div className="p-8 text-red-700">
        <h1 className="font-bold mb-2">Failed to load LD8 data</h1>
    ...
    return (
      <div className="flex items-center justify-center min-h-screen text-slate-500">
        <div className="text-center">
          <div className="animate-pulse text-sm">Loading LD8 precinct data…</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      />
      {state.tab === "dashboard" && (
        <Dashboard rows={computed.rows} district={computed.district} geojson={data.geojson} />
      )}
      {state.tab === "map" && (
      ...
      {state.tab === "report" && (
        <Report rows={computed.rows} district={computed.district} />
      )}
      ...
      <footer className="text-center text-xs text-slate-500 py-6 no-print">
        Path to Victory · NJ LD8 2027 Assembly model · Outputs are modeled, not predictions.
      </footer>
    </div>
  );
}
