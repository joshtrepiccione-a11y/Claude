...
// the precinct list, with no scenario applied. R-perspective: margin = r - d.
...
/** Roll up Sheriff baselines into county-level totals (R-perspective). */
...
export function calculateCandidateResult(
  ...
  candidateId: "sheriffD" | "sheriffR",
): number {
  let v = 0;
  for (const p of precincts) {
    v += candidateId === "sheriffD" ? p.d : p.r;
  }
  return v;
}
