...
// R-perspective: built for O'Donoghue.
...
export function scoreScenarioRealism(
  ...
  const swingPP = Math.abs(a.repSwing - a.demSwing);
  factors.push({
    label: "County swing",
  ...
  factors.push({
    ...
    note: `${improvingPrecincts}/${rows.length} precincts swing >25 R net`,
  });

  // 5. Reliance on opposition strongholds (large lifts in D+10 precincts).
  const strongholdLifts = rows.filter(
    (r) => r.baselineMarginPct <= -10 && r.netVoteOpportunity > 20,
  ).length;
  factors.push({
    label: "Reliance on D strongholds",
    weight: Math.min(15, strongholdLifts * 1.5),
    note: `${strongholdLifts} D+10 precincts asked to swing`,
  });

  // 6. Penalize wins that depend on a razor-thin county margin.
  if (c.electsR && c.scenario.marginVotes < 250 && c.scenario.marginVotes > 0) {
    factors.push({
      label: "Win fragility",
      weight: 8,
