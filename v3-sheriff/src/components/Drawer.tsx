...
import { Card, Pill, fmtNumber, fmtMargin, fmtSigned, partyOf } from "./UI";
    ...
    return (
      <Card title="Precinct detail" subtitle="Click a precinct on the map">
        <div className="text-sm text-slate-500 py-6 text-center">
          ...
          breakdown, turnout sensitivity, and recommended action.
        </div>
      </Card>
    );
  }
  ...
  return (
    <Card
      ...
      right={
        <button
          ...
          onClick={onClose}
          className="text-slate-400 hover:text-slate-700 text-xl leading-none w-11 h-11 sm:w-auto sm:h-auto inline-flex items-center justify-center"
        >
          ×
        </button>
      }
    >
      <div className="space-y-3 text-sm">
        ...
        <div className="text-xs uppercase tracking-wide text-slate-500">Sheriff race (R-perspective)</div>
        <div className="grid grid-cols-2 gap-3">
          <Box label="Baseline margin" value={fmtMargin(row.baselineMarginPct, partyOf(row.baselineMarginPct))} />
          <Box label="Scenario margin" value={fmtMargin(row.scenarioMarginPct, partyOf(row.scenarioMarginPct))} />
          <Box label="Baseline turnout" value={fmtNumber(p.turnout)} />
          <Box label="Scenario turnout" value={fmtNumber(s.turnout)} />
          <Box label="Net R vote swing" value={fmtSigned(row.netVoteSwingR)} />
          <Box label="Turnout sensitivity" value={`${fmtSigned(row.turnoutSensitivity)} pp`} />
        </div>
        ...
        <div>
          ...
          <CandidateRow label={C("sheriffR")!.label} v={p.r} party="R" />
          <CandidateRow label={C("sheriffD")!.label} v={p.d} party="D" />
        </div>
        ...
        <div>
          ...
          <CandidateRow label={C("sheriffR")!.label} v={s.r} party="R" />
          <CandidateRow label={C("sheriffD")!.label} v={s.d} party="D" />
        </div>
