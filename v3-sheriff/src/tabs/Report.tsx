...
export function Report({
  ...
  const memo = useMemo(
    () => generateMemo(preset ?? null, state.assumptions, county, rows, state.baselineId, state.tEnv),
    [preset, state.assumptions, county, rows, state.baselineId, state.tEnv],
  );
  ...
  function shareLink() { navigator.clipboard?.writeText(window.location.href); }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-4 space-y-4">
      <div className="flex items-start justify-between flex-wrap gap-3 no-print">
        <div>
          ...
          <p className="text-sm text-slate-600">
            Plain-English summary of the active baseline and scenario. Updates as you change them.
          </p>
        </div>
      ...
      <Card className="print-page" title={CONTEST.displayName} subtitle={`${CONTEST.subtitle} · Scenario: ${memo.scenarioName}`} right={<Pill tone="gold">Modeled</Pill>}>
