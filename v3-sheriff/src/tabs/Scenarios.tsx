...
export function Scenarios({
  ...
  const comparison = useMemo(() => {
    ...
    return rows.map((r) => {
      const { county, rows: rs } = calculateScenarioResult(precincts, r.assumptions, sensitivityById);
      const realism = scoreScenarioRealism(r.assumptions, county, rs);
      return { ...r, county, realism };
    });
  }, [state.saved, state.assumptions, preset, precincts, sensitivityById]);
    ...
    <div className="max-w-[1500px] mx-auto px-4 py-4 space-y-4">
      <div className="flex items-start justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-sm text-slate-600">Pick a preset, tune assumptions, or ask the model to find a path that holds the seat for O'Donoghue.</p>
        </div>
        <div className="inline-flex border border-slate-300 rounded-md overflow-hidden">
          <button
            ...
            className={`px-3 py-1.5 text-sm min-h-[44px] sm:min-h-0 ${mode === "simple" ? "bg-navy-900 text-white" : "bg-white text-slate-700"}`}
          >Simple</button>
          <button
            onClick={() => { setMode("expert"); dispatch({ type: "setUiMode", m: "expert" }); }}
            className={`px-3 py-1.5 text-sm min-h-[44px] sm:min-h-0 ${mode === "expert" ? "bg-navy-900 text-white" : "bg-white text-slate-700"}`}
          >Expert</button>
        </div>
      </div>

      <Card title="Baseline + turnout environment" subtitle="The baseline that drives the model. Presets may switch this for you.">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {BASELINES.map((b) => {
            const active = state.baselineId === b.id;
            return (
              <button
                key={b.id}
                onClick={() => dispatch({ type: "setBaseline", id: b.id })}
                className={`text-left rounded-lg border p-3 transition ${
                  active ? "border-[#b91c1c] ring-2 ring-red-200 bg-red-50" : "border-slate-200 hover:border-[#b91c1c]"
                }`}
              >
                <div className="flex items-center justify-between">
                  <div className="font-semibold text-slate-800 text-sm">{b.label}</div>
                  {active && <Pill tone="rep">Active</Pill>}
                </div>
        ...
        {state.baselineId === "projected2026" && (
          <div className="mt-3 flex items-center gap-3 text-xs">
            <span className="shrink-0 text-slate-500">2023 off-year</span>
            <input
              type="range"
              min={0}
              max={1}
              ...
              className="flex-1 accent-[#b91c1c]"
              aria-label="Turnout environment factor"
            />
            <span className="shrink-0 text-slate-500">2020 presidential</span>
            <span className="shrink-0 font-mono w-10 text-right">{state.tEnv.toFixed(2)}</span>
          </div>
        )}
        <p className="text-xs text-slate-500 mt-2">Active baseline: <strong>{activeBaseline.label}</strong></p>
      </Card>
        ...
        <Card title="Simple Mode — Preset scenarios">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            <button
              ...
              className={`text-left rounded-lg border p-3 transition ${
                state.scenarioId === "custom" ? "border-[#b91c1c] ring-2 ring-red-200 bg-red-50" : "border-dashed border-slate-300 hover:border-[#b91c1c]"
              }`}
            >
      ...
      <Card
        title="Find a Path to Hold"
        subtitle="Deterministic heuristic — adjusts a small set of high-leverage knobs in a fixed order until O'Donoghue holds the seat."
      >
        <div className="flex flex-wrap items-end gap-3">
          ...
          <Button onClick={onFindPath}>Find a Path to Hold</Button>
          ...
          <table className="min-w-full text-sm">
            <thead className="text-left text-xs uppercase text-slate-500">
              <tr>
                ...
                <th className="px-2 py-2">O'Donoghue Holds?</th>
                <th className="px-2 py-2 text-right">Net R Vote Gain</th>
            ...
            <tbody>
              {comparison.map((r) => (
                <tr key={r.id} className="border-t border-slate-100">
                  ...
                  <td className="px-2 py-2">{fmtMargin(r.county.scenario.marginPct, partyOf(r.county.scenario.marginPct))}</td>
                  <td className="px-2 py-2">{r.county.electsR ? <Pill tone="green">Yes</Pill> : <Pill tone="slate">No</Pill>}</td>
                  <td className="px-2 py-2 text-right">{fmtSigned(r.county.netGain)}</td>
...
function mainLever(a: ScenarioAssumptions): string {
  const items: Array<[string, number]> = [
    ["County R swing", Math.abs(a.repSwing - a.demSwing)],
...
function ExpertControls({
  ...
  const sliders: Array<[keyof ScenarioAssumptions, string, number, number, number, string]> = [
    ["turnoutDelta", "County-wide turnout change", -0.2, 0.2, 0.005, "Δ (frac)"],
    ["repSwing", "Republican (O'Donoghue) swing", -10, 10, 0.1, "pp"],
    ["demSwing", "Democratic (Riggin) swing", -10, 10, 0.1, "pp"],
    ...
    ["vbmMarginSwing", "VBM margin swing (toward R)", -10, 10, 0.1, "pp"],
    ["earlyMarginSwing", "Early vote margin swing (toward R)", -10, 10, 0.1, "pp"],
    ["edMarginSwing", "Election Day margin swing (toward R)", -10, 10, 0.1, "pp"],
  ];
        ...
        {sliders.map(([k, label, min, max, step, unit]) => (
          <div key={String(k)} className="bg-slate-50 rounded-md p-2.5 border border-slate-100">
            ...
            <input
              ...
              className="w-full mt-1 accent-[#b91c1c]"
            />
          </div>
        ))}
      </div>
